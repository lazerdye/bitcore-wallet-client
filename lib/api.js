...
API.prototype._doRequest = function(method, url, args, cb) {
  ...
  this._completeRequest(args, cb);
};

API.prototype._doPeerRequest = function(method, url, args, cb) {
  var absUrl = this.baseUrl + url;
  var args = {
    // relUrl: only for testing with `supertest`
    relUrl: this.basePath + url,
 ...
 * @param {Callback} cb
 */
API.prototype._completeRequest = function(args, cb) {
...
API.prototype._doPeerPostRequest = function(url, args, cb) {
  return this._doPeerRequest('post', url, args, cb);
};
...
API.prototype._doPeerGetRequest = function(url, cb) {
  url += url.indexOf('?') > 0 ? '&' : '?';
  url += 'r=' + _.random(10000, 99999);
  return this._doPeerRequest('get', url, {}, cb);
};
...
API.prototype.createAddressForPeer = function(peerId, cb) {
  this._doPeerPostRequest('/v1/addresses/' + peerId + '/', {}, function(err, address) {
...
API.prototype.getPeerAddresses = function(peerId, cb) {
  this._doPeerGetRequest('/v1/addresses/' + peerId + '/',
...
/**
 * Get the balance for a peer, does not require authentication.
 *
 * @param {Callback} cb
 */
API.prototype.getPeerBalance = function(peerId, cb) {
  this._doPeerGetRequest('/v1/balance/' + peerId + '/', cb);
};
...
API.prototype.getPeerTxHistory = function(peerId, opts, cb) {
  var self = this;
  var args = [];
  if (opts) {
    if (opts.skip) args.push('skip=' + opts.skip);
  ...
  var url = '/v1/txhistory/' + peerId + '/' + qs;
  self._doPeerGetRequest(url, function(err, txs) {
    if (err) return cb(err);
    return cb(null, txs);
  });
};
...
API.prototype.startScanForPeer = function(peerId, opts, cb) {
  var self = this;
  ...
  self._doPeerPostRequest('/v1/addresses/scan/' + peerId + '/', args, function(err) {
    return cb(err);
  });
};
